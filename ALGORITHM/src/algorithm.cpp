...
void Algorithm::run()
{
    std::thread t1(std::bind(&Algorithm::groundMaterialDetect2, this));
    t1.detach();

    //   std::thread t2(std::bind(&Algorithm::hatchDetectClouds2, this));
    //   t2.detach();
}
...
void Algorithm::hatchDetectClouds()
{
    ...
    string folderPath = "/home/ls/Desktop/PortMonitor/ALGORITHM/ship_cloud/";
    ...
    for (int m = 0; m < vecStr.size(); ++m)
    {
        ...
        shipParamIn.roiFile = "/home/ls/Desktop/PortMonitor/ALGORITHM/ROIPointsFile/ShipROIPoints.txt";
...
void Algorithm::hatchDetectClouds2()
{
    ...
    while (true)
    {
        ...
        {
            qWarning() << "算法模块未读取到雷达驱动数据！！！";
            continue;
        }
        double curCraneAngle =process_params_in.protocolData.read_data.angle - floor(process_params_in.protocolData.read_data.angle / 360.0) * 360.0;
       // cout<<"ShipDetection curCraneAngle = "<<curCraneAngle<<endl;

        rtmethod::RTParams rtParams{};
        rtParams.roll=process_params_in.protocolData.read_data.x_roll;
        rtParams.pitch=process_params_in.protocolData.read_data.y_pitch;
        rtParams.heading=curCraneAngle;
        rtParams.x=process_params_in.protocolData.read_data.lidar_x*0.01;
        rtParams.y=process_params_in.protocolData.read_data.lidar_y*0.01;
        rtParams.z=0;//process_params_in.protocolData.read_data.lidar_z*0.01;

        if(curCraneAngle < process_params_in.shipParamIn.min_monitor_angle || curCraneAngle > process_params_in.shipParamIn.max_monitor_angle )
        {
            continue;
        }
        // 舱口矩形检测
        AutomatedPort::ShipDetection shipDetection;
        AutomatedPort::ShipParamIn shipParamIn{};
        shipParamIn.rtParams = rtParams; // 初始旋转平移参数
        shipParamIn.roiFile = "/home/ls/Desktop/PortMonitor/ALGORITHM/ROIPointsFile/ShipROIPoints.txt";

        if (!lidarDriveInterface->get_lidar_data(shipParamIn.cloudSrc)) //  cloud_src;
        {
            continue;
        }
        ...
        {
            recv_lidar_queue_.pop();
            qWarning() << "警告：算法处理完的lidar数据队列超过3！";
        }
    }
}
...
void Algorithm::groundMaterialDetect()
{
    ...
    string folderPath = "/home/ls/Desktop/PortMonitor/ALGORITHM/material_cloud/";
    vector<string> vecStr;
    Util::scan_one_dir(folderPath.c_str(), vecStr);
    for (int m = 0; m < vecStr.size(); ++m)
    {
        ...
        groundParamIn.roiFile = "/home/ls/Desktop/PortMonitor/ALGORITHM/ROIPointsFile/GroundROIPoints.txt";
        groundParamIn.rectangleROIFile = "/home/ls/Desktop/PortMonitor/ALGORITHM/ROIPointsFile/GroundRectanglePoints.txt"; // 物料区划分N*M格子区域
...
void Algorithm::groundMaterialDetect2()
{
    ...
    while (true)
    {
        ...
        double curCraneAngle =process_params_in.protocolData.read_data.angle - floor(process_params_in.protocolData.read_data.angle / 360.0) * 360.0;
       // cout<<"GroundMaterialDetection curCraneAngle = "<<curCraneAngle<<" , "<<process_params_in.protocolData.read_data.angle<<endl;
       ...
       rtmethod::RTParams rtParams{};
        ...
        rtMethod.printRTStruct(rtParams);*/

         if(curCraneAngle < process_params_in.groundParamIn.min_monitor_angle || curCraneAngle > process_params_in.groundParamIn.max_monitor_angle )
        {
            continue;
        }
        ...
        AutomatedPort::GroundMaterialDetection materialDetection;
        AutomatedPort::GroundParamIn groundParamIn{};
        groundParamIn.rtParams = rtParams; // 初始旋转平移参数
        groundParamIn.roiFile = "/home/ls/Desktop/PortMonitor/ALGORITHM/ROIPointsFile/GroundROIPoints.txt";
        groundParamIn.rectangleROIFile = "/home/ls/Desktop/PortMonitor/ALGORITHM/ROIPointsFile/GroundRectanglePoints.txt"; // 物料区划分N*M格子区域

        if (!lidarDriveInterface->get_lidar_data(groundParamIn.cloudSrc)) //  cloud_src;
        {
            continue;
        }

        rtmethod::RTMethod rtMethod;
        rtmethod::RTParams rtParamsSrc{1.9,2.74,0,0,0,0};
        Eigen::Matrix4d rtMat = Eigen::Matrix4d::Identity();
        rtMethod.rtStruct2RTMat_zyx(rtParamsSrc,rtMat);
        PointCloudTPtr cloudRT(new PointCloudT);
        *cloudRT=*groundParamIn.cloudSrc;
        groundParamIn.cloudSrc->clear();
        pcl::transformPointCloud(*cloudRT, *groundParamIn.cloudSrc, rtMat);

        groundParamIn.gridNumX = process_params_in.groundParamIn.gridNumX; // 2
        groundParamIn.gridNumY = process_params_in.groundParamIn.gridNumY;
        // cout<<"groundParamIn.cloudSrc size   IN = "<<groundParamIn.cloudSrc->size()<<endl;
        materialDetection.setGroundParamIn(groundParamIn);
        double st = timespecMs();
        materialDetection.runDetect();
        // cout << "materialDetection = " << timespecMs() - st << " ms " << endl;
        ...
        if (!params_out.groundParamOut.cloudSrcRT->empty()) {
                pcl::PCDWriter writer;
                std::stringstream ss;
                ss << "groundParamOut" << ".pcd";
                writer.write<PointT>(ss.str(), *params_out.groundParamOut.cloudSrcRT+*params_out.groundParamOut.groundCloud, true);
            }
