...
namespace AutomatedPort {
    ...
    void ShipDetection::runDetect() {
        this->init();
         if(shipParamIn.cloudInited->size()<4){
            return;
        }
        this->runHatchDetect();
    ...
    void ShipDetection::init() {
        ...
        rtMethod.rtStruct2RTMat_zyx(shipParamIn.rtParams, rtMat);
        ...
        *shipParamOut.cloudSrcRT=*cloud_srcRT;
    }
...
//        st = Util::timeMs();
    ...
    bool ShipDetection::estimateDeck() {
        ...
        rtMethod.rtMat2RTStruct_zyx(rtMat, rtParams);
