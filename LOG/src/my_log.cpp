...
std::shared_ptr<ParamsEvent<ObserverFunc>> params_event_log;
Log my_log;

void initParamEventLog(std::shared_ptr<ParamsEvent<ObserverFunc>> params_event_)
{
    params_event_log = params_event_;
}
...
void createLogFile()
{
    ...
    if (!log->exists("./info-log/"))
    {
        log->mkdir("./info-log/");
    }
    if (!log->exists("./crittcal-log/"))
    {
        log->mkdir("./crittcal-log/");
    }
    if (!log->exists("./debug-log/"))
    {
        log->mkdir("./debug-log/");
    }
...
void outputMessage(QtMsgType type, const QMessageLogContext &context, const QString &msg)
{
    ...
    if (type == QtCriticalMsg)
    {
        ...
        std::lock_guard<std::mutex> LockGuard(params_event_log->getMylock());

        my_log.flag =true;
        my_log.level = LOGLEVEL::CRITICAL;
        my_log.date = "20221210";
        my_log.msg = msg.toStdString();

        params_event_log->getTotalParams().log =my_log;
        params_event_log->Notify(params_event_log->getTotalParams(), false);
