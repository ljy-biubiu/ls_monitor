...
void LidarDrive128S1::udp_init(int *sock_num, sockaddr_in *sockaddr, int server_port, const std::string ip)
{
    ...
    *sock_num = socket(AF_INET, SOCK_DGRAM, 0);
    if (bind(*sock_num, (struct sockaddr *)sockaddr, sizeof(*sockaddr)) < 0)
    {
        ...
        params_event->getTotalParams().alarm_status.communicate_status.bits.lidar_drive = 0;
...
void LidarDrive128S1::globalParamsCallback(TotalParams msg, bool save_flag)
{
...
{
    PointCloudTPtr tCloud(new PointCloudT);
